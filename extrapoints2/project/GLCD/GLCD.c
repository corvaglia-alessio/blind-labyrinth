...
**
** Descriptions:		Draw Triangle
**
** parameters:			x0 e y0 are the coordinates of the first vertex 
												at the top for e and w
												at bottom left for s and n
												d is the base size
												o is the direction 's', 'n', 'w', 'e';
** Returned value:		None
**
**************************/
void LCD_DrawTriangle(uint16_t x0, uint16_t y0, uint32_t d, uint32_t o , uint16_t color )
{
	uint32_t cnt, x1;
	char odd = ((d&1) == 1)?1:0;
	
    if (o == 'e') {
		for(cnt = 0, x1 = 2; cnt<d; cnt++) {
			if (cnt <= ((d/2)+odd)) {
				LCD_DrawLine(x0, y0 + cnt, x0+cnt, y0 + cnt, color);
				x1 = cnt;
			}
			else {
				x1--;
				LCD_DrawLine(x0, y0 + cnt, x0 + x1, y0 + cnt, color);
			}
		}
	}
	...
	if (o == 's') {
		for(cnt = 0; cnt<((d/2)+odd); cnt++) {
				LCD_DrawLine(x0 + cnt, y0 + cnt, x0 + d - cnt, y0 + cnt, color);
		}
	}
